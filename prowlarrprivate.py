...
def load_configuration():
    ...
    # do some checks
    if any(item not in CONFIG_DATA for item in ['api_key', 'url', 'indexerIds']):
        CONFIG_DATA['malformed'] = True
...
class prowlarrprivate(object):
    name = 'prowlarr (private)'
    url = CONFIG_DATA['url'] if CONFIG_DATA['url'][-1] != '/' else CONFIG_DATA['url'][:-1]
    api_key = CONFIG_DATA['api_key']
    indexerIds = CONFIG_DATA['indexerIds']
    ...
    def search(self, what, cat='all'):
        ...
        indexerIdArray = self.indexerIds.split(',')

        # Create the new string with the desired format
        indexerIdsQuery = '&'.join([f'indexerIds={value}' for value in indexerIdArray])

        # Add the leading '&'
        indexerIdsQuery = '&' + indexerIdsQuery

        if category is not None:
            prowlarr_url = self.url + "/api/v1/" + 'search?query=' + what.replace(' ', '+') + '&apikey=' + self.api_key + indexerIdsQuery + "&categories=" + str(category)
        else:
            prowlarr_url = self.url + "/api/v1/" + 'search?query=' + what.replace(' ', '+') + '&apikey=' + self.api_key + indexerIdsQuery
        ...
        for result in x:
            ...
            # get the indexer search engine url
            indexerSiteQueryUrl = self.url + "/api/v1/indexer/" + str(result.get("indexerId")) + "?apikey=" + self.api_key
            indexerSiteResponse = self.get_response(indexerSiteQueryUrl)
            indexerSiteResult = json.loads(indexerSiteResponse)
            base_url_label = next((field.get('value', indexerSiteResult['indexerUrls'][0]) for field in indexerSiteResult['fields'] if field['name'] == 'baseUrl'), '')
            indexerSite = base_url_label

            publishDate = datetime.strptime(result.get('publishDate'), "%Y-%m-%dT%H:%M:%SZ")

            res['name'] = result.get('title')
            ...
            res['pub_date'] = int(publishDate.timestamp())
